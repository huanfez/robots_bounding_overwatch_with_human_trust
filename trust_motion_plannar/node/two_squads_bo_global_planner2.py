...
# plot posterior
def plot_posterior_pdf(Beta0_itr, Sigma0_itr, iteration, ax, x):
...
def plot_crecible_interval(posterior_list_beta, posterior_list_sigma):
...
if __name__ == '__main__':
    try:
        ...
        Beta0_itr = np.asarray([0.25, 0.25, 0.25, 0.25, 0.25])
        ...
        ax[0, 0].plot(x, norm.pdf(x, Beta0_itr[0], np.sqrt(Sigma0_itr[0, 0])), 'r-', lw=1, label=r'$\beta_{-1}$')
        ax[0, 0].plot(x, norm.pdf(x, Beta0_itr[1], np.sqrt(Sigma0_itr[1, 1])), 'g-', lw=1, label=r'$\beta_0$')
        ax[0, 0].plot(x, norm.pdf(x, Beta0_itr[2], np.sqrt(Sigma0_itr[2, 2])), 'b-', lw=1, label=r'$\beta_1$')
        ax[0, 0].plot(x, norm.pdf(x, Beta0_itr[3], np.sqrt(Sigma0_itr[3, 3])), 'y-', lw=1, label=r'$\beta_2$')
        ax[0, 0].plot(x, norm.pdf(x, Beta0_itr[4], np.sqrt(Sigma0_itr[4, 4])), 'c-', lw=1, label=r'$b$')
        ...
        for iteration in range(0, 6):
            ...
            if iteration == 0:
                optimo_cell_path = common_parameters.candidate_cell_path_list[0]
            else:
                optimo_cell_path, f = cps2.optimal_cell_path_pi2(Beta0_itr, Sigma0_itr,
            ...
            # mcmc sampling
            samples_x_1toK, samples_Beta, samples_delta_w_square, samples_delta_v_square, Beta0_itr, Sigma0_itr, \
                a0_itr, b0_itr, c0_itr, d0_itr = pmc2.iterated_sampling(y_1toK, Z_1toK, Beta0_itr, Sigma0_itr, a0_itr,
                                                                        b0_itr, c0_itr, d0_itr, Alpha, iters=5000)
            ...
            if iteration == 5:
                bic_result, neg_log_prob = bic_score(Beta0_itr, Sigma0_itr, a0_itr, b0_itr, c0_itr, d0_itr,
                                                     samples_x_1toK, Z_1toK, y_1toK)
                print "BIC score:", bic_result, neg_log_prob

        # plotting final credible intervals
        plot_crecible_interval(posterior_list_beta, posterior_list_sigma)
        plt.show()
