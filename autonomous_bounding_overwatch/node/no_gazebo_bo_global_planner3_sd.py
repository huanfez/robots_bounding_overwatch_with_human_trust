...
if __name__ == '__main__':
    try:
        ...
        cell_path_list = common_parameters.candidate_cell_path_list
        ...
        for iteration in range(0, 21):
            ...
            optimo_cell_path = cell_path_list[iteration % 5]
        ...
        for i in range(0, 5):
            ax2[i].fill_between(iter_num, np.array(posterior_list_beta_lb)[:, i], np.array(posterior_list_beta_ub)[:, i],
                                alpha=0.2)
            ax2[i].plot(iter_num, np.ones(21)*common_parameters.beta_true3[i])
