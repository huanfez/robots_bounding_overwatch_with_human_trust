...
def predict_xall(Z_1toK, Beta):
    ...
    for k in range(0, K):
        for i in range(0, I):
            if i == 0:
                Z_tilde_1toK[k, i, 0] = 0.0
            else:
                Z_tilde_1toK[k, i, 0] = xall_1toK[k, i-1]

            if k == 0:
                Z_tilde_1toK[k, i, 1] = xall_0[0, i]
            else:
                Z_tilde_1toK[k, i, 1] = xall_1toK[k - 1, i]
...
# The first iteration
def first_iteration_mcmc(Z_1toK, Beta0=np.asarray([0.25, 0.25, 0.25, 0.25, 0.25]),
                         Sigma0=np.diag([1e5, 1e5, 1e5, 1e5, 1e5]), a0=4.0, b0=1.0,
                         c0=2.0, d0=1.0):
    """
...
def sample_xall(y_1toK, Z_1toK, xall_1toK_s, Beta_s, xi_square_s, Alpha, xi2_square_s,
    ...
    K = len(y_1toK)
    Z_flatten_1toK = (Z_1toK[:, :, 2:]).reshape(K, 9)  # state space's inputs

    # state space model: parameter matrix
    Phi11 = np.diag([Beta_s[1], Beta_s[1], Beta_s[1]])
    Phi11[1,0] = Phi11[2,1] = Beta_s[0]
    ...
    Gamma0 = np.concatenate((Beta_s[2:], np.zeros(6)))
    Gamma1 = np.concatenate((np.zeros(3), Beta_s[2:], np.zeros(3)))
    Gamma2 = np.concatenate((np.zeros(6), Beta_s[2:]))
    ...
    Q[0:3, 0:3] = np.diag([xi_square_s, xi_square_s, xi_square_s])
    R = np.diag([xi2_square_s, xi2_square_s, xi2_square_s])

    # print (xstate_all_1toK_s, Z_flatten_1toK,
...
def sample_delta_w_square(Z_tilde_1toK_s_flatten, xall_1toK_s_flatten, Beta_s, a0, b0):
    ...
    a = a0 + K3 / 2.0 - 1.0  # posterior
    b = b0 + np.matmul(error_1toK_s.T, error_1toK_s) / 2.0  # posterior
    delta_w_square_s = invgamma.rvs(a, 0.0, b)
    return delta_w_square_s, a, b
...
def iterated_sampling(y_1toK, Z_1toK, Beta0_itr, Sigma0_itr, a0_itr, b0_itr, c0_itr, d0_itr, Alpha, iters=5000):
    ...
    Z_tilde_1toK_s[:, :, 0:2] = 0.0
    Z_tilde_1toK_s[:, :, 4] = 1.0
    ...
    for itr in range(0, iters):
        ...
        for k in range(K - 1, -1, -1):
            for i in range(0, 3):
                if i == 0:
                    ...
                    Z_tilde_1toK_s[k, i, 0] = xall_1toK_s[k-1, i-1]

                if k == 0:
                    Z_tilde_1toK_s[k, i, 1] = 0.0  # need to carefully tune
                else:
                    Z_tilde_1toK_s[k, i, 1] = xall_1toK_s[k - 1, i]
        ...
        # Beta: multivariate normal
        Beta_s, E_beta, V_beta = sample_Beta(Z_tilde_1toK_s_flatten, xall_1toK_s_flatten, delta_w_square_s,
                                             Beta0=Beta0_itr, Sigma0=Sigma0_itr)
        samples_Beta.append(np.copy(Beta_s))

        # residue: inverse gamma
        delta_w_square_s, a_dw, b_dw = sample_delta_w_square(Z_tilde_1toK_s_flatten, xall_1toK_s_flatten, Beta_s,
                                                             a0=a0_itr, b0=b0_itr)
        samples_delta_w_square.append(np.copy(delta_w_square_s))

        # Observation: residue 2
        delta_v_square_s, c_dv, d_dv = sample_delta_v_square(y_1toK, xall_1toK_s, xall_0, Alpha, c0=c0_itr, d0=d0_itr)
        samples_delta_v_square.append(np.copy(delta_v_square_s))

    return samples_x_1toK, samples_Beta, samples_delta_w_square, samples_delta_v_square, E_beta, V_beta, a_dw, b_dw, c_dv, d_dv
...
def mean_value_model_parameters_(Beta_mean, Beta_var, delta_w_square_mean, delta_w_square_var,
                                 delta_v_square_mean, delta_v_square_var):
    print "posterior of beta:"
    for jj in range(0, 5):
        print "mean:", Beta_mean[jj], "variance:", Beta_var[jj][jj], "creditable interval:", Beta_mean[jj] - \
              2.5 * np.sqrt(Beta_var[jj][jj]), Beta_mean[jj] + 2.5 * np.sqrt(Beta_var[jj][jj])

    # print "posterior of delta_w_square:"
    # print "mean:", delta_w_square_mean, "variance:", delta_w_square_var, "creditable interval:", \
    #       delta_w_square_mean - 2.5 * np.sqrt(delta_w_square_var), delta_w_square_mean + \
    #       2.5 * np.sqrt(delta_w_square_var)
    #
    # # traces of posterior delta_v_square
    # print "posterior of delta_v_square:"
    # print "mean:", delta_v_square_mean, "variance:", delta_v_square_var, "creditable interval:", \
    #       delta_v_square_mean - 2.5 * np.sqrt(delta_v_square_var), delta_v_square_mean + \
    #       2.5 * np.sqrt(delta_v_square_var)
