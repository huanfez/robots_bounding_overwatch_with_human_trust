...
def first_iteration_mcmc(Z_1toK, Beta0=np.asarray([0.25, 0.25, 0.25, 0.25, 0.25]),
                         Sigma0=np.diag([1e5, 1e5, 1e5, 1e5, 1e5]), a0=4.0, b0=np.diag([1.0, 1.0, 1.0]),
                         c0=2.0, d0=1.0):
    """
    ...
    delta_w_square_s = invwishart.rvs(a0, b0)  # initial sampling
...
def sample_xall(y_1toK, Z_1toK, xall_1toK_s, Beta_s, xi_square_s, Alpha, xi2_square_s,
    ...
    Phi11[1,0] = Phi11[2,1] = Beta_s[0] * Beta_s[1]
    ...
    Gamma1 = np.concatenate((Beta_s[2:] * Beta_s[0], Beta_s[2:], np.zeros(3)))
    Gamma2 = np.concatenate((np.zeros(3), Beta_s[2:] * Beta_s[0], Beta_s[2:]))
...
# sample_Beta
def sample_Beta2(Z_tilde_1toK_s, xall_1toK_s, delta_w_square_s, Beta0, Sigma0):
    z_shape = Z_tilde_1toK_s.shape
    Z_tilde_1toK_s_T = np.transpose(Z_tilde_1toK_s, (0,2,1))
    Z_temp1 = np.matmul(Z_tilde_1toK_s_T, inv(delta_w_square_s))

    Z_square = np.matmul(Z_temp1, Z_tilde_1toK_s)
    V = inv(inv(Sigma0) + np.sum(Z_square, axis=0))

    xall_1toK_s_reshape = xall_1toK_s.reshape((z_shape[0], z_shape[1], 1))
    E_ = np.matmul(inv(Sigma0), Beta0) + np.sum(np.matmul(Z_temp1, xall_1toK_s_reshape), axis=0).flatten()
...
def sample_delta_w_square(Z_tilde_1toK_s, xall_1toK_s, Beta_s, a0, b0):
    K3 = len(Z_tilde_1toK_s)
    error_1toK_s = xall_1toK_s - np.matmul(Z_tilde_1toK_s, Beta_s)  # vector operation
    a = a0 + K3 - 4  # posterior
    b = b0 + np.matmul(error_1toK_s.T, error_1toK_s)  # posterior
    delta_w_square_s = invwishart.rvs(a, b)
...
def iterated_sampling(y_1toK, Z_1toK, Beta0_itr, Sigma0_itr, a0_itr, b0_itr, c0_itr, d0_itr, Alpha, iters=5000):
    ...
    for itr in range(0, iters):
        ...
        for k in range(K - 1, -1, -1):
            for i in range(0, 3):
                if i == 0:
                    ...
                    Z_tilde_1toK_s[k, i, 0] = xall_1toK_s[k, i-1]
        ...
        # Beta: multivariate normal
        Beta_s, E_beta, V_beta = sample_Beta2(Z_tilde_1toK_s, xall_1toK_s, delta_w_square_s,
                                              Beta0=Beta0_itr, Sigma0=Sigma0_itr)
        samples_Beta.append(np.copy(Beta_s))

        # residue: inverse gamma
        delta_w_square_s, a_dw, b_dw = sample_delta_w_square(Z_tilde_1toK_s, xall_1toK_s, Beta_s,
                                                             a0=a0_itr, b0=b0_itr)
